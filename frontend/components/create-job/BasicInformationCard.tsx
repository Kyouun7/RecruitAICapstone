...
export default function BasicInformationCard({ jobTitle, employmentType, workSetup, location, onFieldChange }: BasicInformationCardProps) {
  return (
    <section className="bg-surface-lowest rounded-xl p-5 sm:p-8 shadow-[0_12px_32px_-4px_rgba(25,28,29,0.06)]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="md:col-span-2">
          ...
          <input type="text" value={jobTitle} onChange={(e) => onFieldChange('jobTitle', e.target.value)}
        ...
        <div>
          ...
          <select value={employmentType} onChange={(e) => onFieldChange('employmentType', e.target.value)}
        ...
        <div>
          <label className="block text-xs font-semibold text-on-surface-variant uppercase tracking-wider mb-2">Sistem Kerja</label>
          <div className="flex gap-3">
            {WORK_SETUP_OPTIONS.map((option) => (
              <button key={option} type="button" onClick={() => onFieldChange('workSetup', option)}
                className={`flex-1 py-3 px-4 rounded-lg font-medium text-sm transition-all duration-150 cursor-pointer ${workSetup === option ? 'bg-white border border-primary text-primary shadow-sm' : 'bg-surface-low text-on-surface-variant hover:bg-surface-container'}`}>
                {option}
              </button>
            ))}
          </div>
        ...
        <div className="md:col-span-2">
          ...
          <div className="relative">
            ...
            <input type="text" value={location} onChange={(e) => onFieldChange('location', e.target.value)}
